...
<body>
	...
	<table>
		<tr>
			...
			<th id="Title" style="font-size:15px"><center><b>{{ Helper::FormatInvWh($inv) }}</b></center></th>
		</tr>
	</table>		
	<p></p>
	<table>	
			...
			@endphp
					...
					@foreach ($detail as $dtl)
						...
						<tr>
							...
							<td width="25%" style="text-align:center;">{{ Helper::Rupiah($dtl->tarif) }}</td> 
...
<table>
	<tr>
		<td> <a href="{{ route('wh.generate.invoicecn',[$manifest->kd_vsl]) }}">Back</a> </td>
		<td>
			<form action="{{ route('wh.invoice.genpdf-cn.print',[$manifest->kd_vsl]) }}" method="post">  
   {{ csrf_field() }}
